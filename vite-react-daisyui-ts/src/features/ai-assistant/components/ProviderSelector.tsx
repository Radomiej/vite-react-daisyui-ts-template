...
interface ProviderSelectorProps {
  ...
  onUpdateProvider: (oldName: string, newProvider: AIProviderConfig) => void;
}
...
export const ProviderSelector: React.FC<ProviderSelectorProps> = ({
  ...
  const [editProvider, setEditProvider] = useState<Partial<AIProviderConfig>>({});
  ...
  const handleSubmitEdit = (e: React.FormEvent, oldName: string) => {
    e.preventDefault();
    if (editProvider.name && editProvider.testUrl && editProvider.productionUrl) {
      onUpdateProvider(oldName, editProvider as AIProviderConfig);
      setEditingProvider(null);
      setEditProvider({});
    }
  };
    ...
    <div className="p-4">
      ...
      <div className="space-y-2">
        {providers.map((provider) => {
          ...
          if (isEditing) {
            return (
              <div key={provider.name} className="card bg-base-300 shadow-xl">
                <div className="card-body p-4">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="card-title text-sm">Edit Provider</h3>
                    <button
                  ...
                  <form onSubmit={(e) => handleSubmitEdit(e, provider.name)} className="space-y-3">
                    <div className="form-control">
                      <label className="label py-1">
                        <span className="label-text text-xs">Name</span>
                      </label>
                    ...
                    <div className="form-control">
                      ...
                      <select
                        ...
                        onChange={(e) => setEditProvider({ ...editProvider, type: e.target.value as 'litellm' | 'n8n' })}
                      >
                        <option value="litellm">LiteLLM</option>
                        <option value="n8n">N8n</option>
                      </select>
                    ...
                    <div className="form-control">
                      ...
                      <input
                        type="url"
                        className="input input-sm input-bordered"
                        value={editProvider.testUrl || ''}
                        onChange={(e) => setEditProvider({ ...editProvider, testUrl: e.target.value })}
                        required
                      />
                    </div>
                    ...
                    <div className="form-control">
                      ...
                      <input
                        type="url"
                        className="input input-sm input-bordered"
                        value={editProvider.productionUrl || ''}
                        onChange={(e) => setEditProvider({ ...editProvider, productionUrl: e.target.value })}
                        required
                      />
                    </div>
